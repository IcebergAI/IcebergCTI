...
- **Knowledge graph — tag aliases (done, roadmap 2a)** — the named-threat tag kinds (ACTOR/MALWARE/CAMPAIGN) carry a structured `aliases` list and search is alias-aware, so APT28 / Fancy Bear / Sofacy resolve to one entity. First rung of Priority 2; see *Tagging & search* above. **Next on that roadmap:** 2b (entity attribution profile — turn `/tags/{id}` into a proper actor profile) → 2c (entity relationships); or §1c stretch — ACH as a second analytic model.
