...
- **Machine-readable interop** — **ATT&CK Navigator layer export is shipped** (backlog A: per-report + per-entity `.json` + a coverage heatmap), **STIX 2.1 report bundle export is shipped** for report/taxonomy SDOs, and **read-only TAXII serving is shipped** for the published-report collection with incremental pull filters, and **inbound TAXII/MISP pull into notebooks is shipped** (`services/inbound.py`). Richer STIX validation/enrichment remains.
...
| B | [#29](../../issues/29) | **STIX 2.1 / TAXII interop** — partial ✅ | STIX 2.1 bundle export is shipped for a visible report plus report/taxonomy SDOs (threat actors, malware, campaigns, ATT&CK attack-patterns and sectors), and a read-only TAXII-shaped `published-reports` collection serves published objects with `added_after`, `limit`, `next`, `match[type]`, and `match[id]` pull filters. Relationship/SRO enrichment and stricter STIX validation remain follow-ups. | High / Medium — foundation shipped |
| C | [#30](../../issues/30) | **Dissemination channels + subscription matching** — partial ✅ | Publication webhooks support stable generic JSON plus opt-in Slack Block Kit and Microsoft Teams MessageCard envelopes; stakeholder tag-subscription matching and portal subscription management in Preferences are shipped. | Medium / Medium — foundation shipped |
...
| E | [#32](../../issues/32) | **PAP (Permissible Actions Protocol)** beside TLP | **Not planned.** The feature was closed as Won't Do because adoption does not justify additional classification/UI friction; reconsider only if ecosystem use changes. | — |
...
| I | [#50](../../issues/50) | **Inbound collection — external feed ingestion into notebooks** — partial ✅ | RSS/Atom feed source management, bounded safe pulls (opt-in scheduled polling via the durable job outbox), portal triage, discard and promote-to-notebook-source are shipped, as is writer-triggered TAXII/MISP pull into notebooks. Relevance triage remains a follow-up. | High / Large — foundation shipped |
...
1. **AI review workflow.** Largely shipped: AI-assisted IOC extraction (#95 — the notebook Indicators review list) and the report-editor **AI review** dock (judgement drafts, tag suggestions, challenge notes, applied through the ordinary save flows). The remaining UX gap is the feed-reader IOC surface (a feed item becomes a `Source` on send-to-notebook, then reuses the same flow).
2. **Richer STIX interop.** TAXII serving and inbound TAXII/MISP pull are shipped; validation tooling, richer external references and optional relationship/SRO enrichment are the remaining interop follow-ups. (Full ATT&CK import shipped as `iceberg-import-attack`.)

**Loose follow-ups:** tag merge/rename tooling and a semantic embedding backend for related reports. (Durable background jobs for notifications, webhooks and RSS polling shipped as the `OutboxJob` outbox + `iceberg-worker`.)
